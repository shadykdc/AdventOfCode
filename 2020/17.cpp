...
public:
    ...
    bool is_neighbor(int _x, int _y, int _z)
    {
        return abs(x - _x) <= 1 && abs(y - _y) <= 1 && abs(z - _z) <= 1;
    }
};
...
public:
    ...
    void activate_pt(int x, int y, int z);
    bool deactivate_pt(int x, int y, int z);
    bool pt_is_active(int x, int y, int z);
    size_t count_active_nearby(int x, int y, int z);
...
void Space::activate_pt(int x, int y, int z)
{
    if (pt_is_active(x, y, z)) return;
    Point p(x, y, z);
...
bool Space::pt_is_active(int x, int y, int z)
{
    Point p(x, y, z);
...
bool Space::deactivate_pt(int x, int y, int z)
{
    Point p(x, y, z);
...
size_t Space::count_active_nearby(int px, int py, int pz)
{
    size_t count = 0;
...
void iterate(Space* space)
{
    ...
    for (int x = space->x_min-1; x <= space->x_max+1; x++)
    {
        for (int y = space->y_min-1; y <= space->y_max+1; y++)
        {
            ...
            {
                size_t count = space->count_active_nearby(x, y, z);
                if (space->pt_is_active(x, y, z) && count != 2 && count != 3)
                    new_space.deactivate_pt(x, y, z);
                else if (count == 3)
                    new_space.activate_pt(x, y, z);
            }
        }
    }
    *space = new_space;
